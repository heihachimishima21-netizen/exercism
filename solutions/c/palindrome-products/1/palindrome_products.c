...
bool palindrome(int n){
    ...
    for (int i = 0; i < count; i++)
        reverse[i] = string[count - i - 1];
    reverse[count] = '\0';
    int m = atoi(reverse);
    return m == n;
    // could count digits with a while loop and then use a for loop and / & % and stuff to check opposite ends digits and working towards the middle
}
