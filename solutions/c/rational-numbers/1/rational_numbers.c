...
int gcd(rational_t r){
    int a = abs(r.numerator), b = abs(r.denominator), rem;
    while (b != 0){
        rem = a%b;
        a = b;
        b = rem;
...
rational_t exp_rational(rational_t r, int16_t n){
    rational_t result = (n>0) ? (rational_t){round(pow(r.numerator, n)), round(pow(r.denominator, n))} : (rational_t){round(pow(r.denominator, -n)), round(pow(r.numerator, -n))};
    return reduce(result);
}

float exp_real(uint16_t x, rational_t r){
    float result = pow(pow(x,r.numerator), 1.0/r.denominator);
    return result;
}
